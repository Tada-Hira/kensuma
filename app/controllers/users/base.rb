          ...
          worker.to_json(
            ...
            include: {
              ...
              worker_skill_trainings:          {
                only: [:skill_training_id, :images] # 中間テーブル(技能講習マスタ)
              },
              worker_special_educations:       {
                only: [:special_education_id, :images] # 中間テーブル(特別教育マスタ)
              },
              worker_safety_health_educations: {
                only: [:safety_health_education_id, :images] # 中間テーブル(安全教育マスタ)
              },
              worker_licenses:                 {
                only: [:license_id, :images] # 中間テーブル(免許マスタ)
              }
            }
          )
        )
